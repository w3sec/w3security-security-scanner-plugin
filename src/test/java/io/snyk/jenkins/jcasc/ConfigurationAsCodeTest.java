...
public class ConfigurationAsCodeTest {
  ...
  public void should_support_configuration_as_code() throws Exception {
    W3SecurityInstallation.W3SecurityInstallationDescriptor descriptor = ExtensionList.lookupSingleton(
      W3SecurityInstallation.W3SecurityInstallationDescriptor.class
    );
    ...
    assertEquals(
      PlatformItem.LINUX,
      ((W3SecurityInstaller) installers.get(0)).getPlatform()
    );
    assertEquals(
      Long.valueOf(36l),
      ((W3SecurityInstaller) installers.get(0)).getUpdatePolicyIntervalHours()
    );
    assertEquals(
      "1.947.0",
      ((W3SecurityInstaller) installers.get(0)).getVersion()
    );
  }
}
