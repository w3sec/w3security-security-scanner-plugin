...
public class CommandLineTest {
  ...
  public void shouldIncludeCommand() {
    ...
    ArgumentListBuilder result = CommandLine.asArgumentList(
      "/usr/bin/w3security",
      Command.MONITOR,
      config,
      env
    ...
    assertThat(
      result.toList(),
      equalTo(asList("/usr/bin/w3security", "monitor"))
    );
  }
  ...
  public void shouldIncludeJsonForTest() {
    ...
    assertThat(
      result.toList(),
      equalTo(asList("/usr/bin/w3security", "test", "--json"))
    );
  }
  ...
  public void shouldReplaceMacrosWithEnvironmentVariables() {
    ...
    W3SecurityConfig config = Mockito.mock(W3SecurityConfig.class);
    when(config.getAdditionalArguments())
      .thenReturn("image:$BRANCH_NAME-${BUILD_NUMBER}");
