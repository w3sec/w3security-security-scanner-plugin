...
public interface W3SecurityApiToken extends StandardCredentials {
  ...
  static String getToken(W3SecurityContext context, String w3securityTokenId) {
    return Optional
      .ofNullable(w3securityTokenId)
      .map(Util::fixEmptyAndTrim)
      .map(id ->
        Optional
          .ofNullable(
            findCredentialById(id, W3SecurityApiToken.class, context.getRun())
          )
          .orElseThrow(() ->
            new RuntimeException(
              "W3Security API token with Credential ID '" +
              w3securityTokenId +
              "' was not found."
            )
          )
      ...
      .map(Optional::of)
      .orElseGet(() ->
        Optional.ofNullable(context.getEnvVars().get(W3SECURITY_TOKEN_ENV_KEY))
      )
      .orElseThrow(() ->
        new RuntimeException(
