...
public class CommandLine {
  ...
  public static ArgumentListBuilder asArgumentList(
    String executablePath,
    Command command,
    W3SecurityConfig config,
    EnvVars env
  ) {
    Function<String, String> replaceMacroWithEnv = str ->
      Util.replaceMacro(str, env);
  ...
  public static Map<String, String> asEnvVars(
    String w3securityToken,
    EnvVars envVars
  ) {
    HashMap<String, String> result = new HashMap<>(envVars);
    Optional
      .ofNullable(w3securityToken)
      .ifPresent(token -> result.put(W3SECURITY_TOKEN_ENV_KEY, token));
    result.put(
      "W3SECURITY_INTEGRATION_NAME",
      PluginMetadata.getIntegrationName()
    );
    result.put(
      "W3SECURITY_INTEGRATION_VERSION",
      PluginMetadata.getIntegrationVersion()
    );
    return result;
  }
