...
import org.w3security.Symbol;

public class W3SecurityInstallation
  extends ToolInstallation
  implements
    EnvironmentSpecific<W3SecurityInstallation>,
    NodeSpecific<W3SecurityInstallation> {

  private static final Logger LOG = LoggerFactory.getLogger(
    W3SecurityInstallation.class
  );

  @DataBoundConstructor
  public W3SecurityInstallation(
    @Nonnull String name,
  ...
  public W3SecurityInstallation forEnvironment(EnvVars environment) {
    return new W3SecurityInstallation(
      getName(),
  ...
  @Override
  public W3SecurityInstallation forNode(@Nonnull Node node, TaskListener log)
  ...
  public String getW3SecurityExecutable(@Nonnull Launcher launcher)
    throws IOException, InterruptedException {
    return resolveExecutable(launcher, "w3security");
  }

  public String getReportExecutable(@Nonnull Launcher launcher)
    throws IOException, InterruptedException {
    return resolveExecutable(launcher, "w3security-to-html");
  }

  private String resolveExecutable(Launcher launcher, String executableName)
    throws IOException, InterruptedException {
    String home = Optional
      .ofNullable(getHome())
      .orElseThrow(() ->
        new RuntimeException(
          "Failed to find W3Security Executable. Installation Home is not configured."
        )
      );

    Platform platform = null;
    W3SecurityStepBuilderDescriptor descriptor = Jenkins
      .get()
      .getDescriptorByType(W3SecurityStepBuilderDescriptor.class);
    W3SecurityInstallation w3securityInstallation = Stream
      .of(descriptor.getInstallations())
      .filter(installation -> installation.getName().equals(this.getName()))
      .findFirst()
      .orElse(null);
    if (w3securityInstallation != null) {
      PlatformItem installerPlatform = getW3SecurityInstallerPlatformIfDefined(
        w3securityInstallation
      );
      platform = PlatformItem.convert(installerPlatform);
    }

    return Optional
      .ofNullable(launcher.getChannel())
      .orElseThrow(() ->
        new RuntimeException(
          "Failed to find W3Security Executable. Build Node does not support channels."
        )
      )
  ...
  @Nonnull
  private PlatformItem getW3SecurityInstallerPlatformIfDefined(
    ...
    try {
      if (
        properties.size() == 1 &&
        properties.get(0) instanceof InstallSourceProperty
      ) {
        DescribableList<ToolInstaller, Descriptor<ToolInstaller>> installers =
          ((InstallSourceProperty) properties.get(0)).installers;
        if (
          installers.size() == 1 &&
          installers.get(0) instanceof W3SecurityInstaller
        ) {
    ...
    LOG.warn(
      "Could not find defined installer architecture, will return 'AUTO'"
    );
    return PlatformItem.AUTO;
  }

  public static W3SecurityInstallation install(
    W3SecurityContext context,
    String name
  ) throws IOException, InterruptedException {
    Node node = Optional
      .ofNullable(context.getWorkspace().toComputer())
      .map(Computer::getNode)
      .orElseThrow(() ->
        new RuntimeException(
          "Failed to install W3Security. W3Security can only be installed on a Build Node."
        )
      );
    ...
    return Stream
      ...
      .orElseThrow(() ->
        new RuntimeException(
          "Failed to install W3Security. Installation named '" +
          name +
  ...
  private static class ResolveExecutable
    ...
    public String call() {
      ...
      LOG.info(
        "'{}' platform will be used by resolving W3Security Executable",
        platform
      );

      String filename = "w3security".equals(executableName)
        ? platform.w3securityWrapperFileName
        : platform.w3securityToHtmlWrapperFileName;

      final Path executable = Paths
        .get(home)
        .resolve(filename)
        .toAbsolutePath();
      if (!executable.toFile().exists()) {
        throw new RuntimeException(
          "Failed to find W3Security Executable. Executable does not exist. (" +
  ...
  @Extension
  @Symbol("w3security")
  public static class W3SecurityInstallationDescriptor
    extends ToolDescriptor<W3SecurityInstallation> {
    ...
    public List<? extends ToolInstaller> getDefaultInstallers() {
      return Collections.singletonList(
        new W3SecurityInstaller(null, null, null, null)
      );
    }
