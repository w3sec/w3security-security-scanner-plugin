...
public class W3SecurityInstaller extends ToolInstaller {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(
    W3SecurityInstaller.class.getName()
  );
  ...
  public FilePath performInstallation(
    ...
    logger.println(
      "Installing W3Security (" + fixEmptyAndTrim(version) + ")..."
    );
    return downloadW3SecurityBinaries(expected, node);
  }

  private boolean isUpToDate(FilePath expectedLocation)
    throws IOException, InterruptedException {
    ...
    try {
      ...
      LOG.error(
        ".timestamp file is corrupt and cannot be read and will be reset to 0."
      );
      timestampFromFile = 0;
    }
  ...
  private FilePath downloadW3SecurityBinaries(FilePath expected, Node node) {
    try {
      LOG.info(
        "Installing W3Security '{}' on Build Node '{}'",
        version,
        node.getDisplayName()
      );
      ...
      if (nodeChannel == null) {
        throw new IOException(
          format("Build Node '%s' is offline.", node.getDisplayName())
        );
      }
      ...
      URL w3securityDownloadUrl = DownloadService.getDownloadUrlForW3Security(
        version,
        platform
      );
      URL w3securityToHtmlDownloadUrl = DownloadService.getDownloadUrlForW3SecurityToHtml(
        "latest",
        platform
      );
      ...
      nodeChannel.call(
        new Downloader(
          w3securityDownloadUrl,
          expected.child(platform.w3securityWrapperFileName)
        )
      );
      nodeChannel.call(
        new Downloader(
          w3securityToHtmlDownloadUrl,
          expected.child(platform.w3securityToHtmlWrapperFileName)
        )
      );
      expected
        .child(INSTALLED_FROM)
        .write(w3securityDownloadUrl.toString(), UTF_8.name());
      expected
        .child(TIMESTAMP_FILE)
        .write(valueOf(Instant.now().toEpochMilli()), UTF_8.name());
  ...
  @Extension
  public static final class W3SecurityInstallerDescriptor
    extends ToolInstallerDescriptor<W3SecurityInstaller> {

    @Nonnull
    @Override
    public String getDisplayName() {
      return "Install from w3security.io";
    }
      ...
      if (!Functions.isWindows() && downloadedFile.isFile()) {
        ...
        if (!result) {
          throw new RuntimeException(
            format(
              "Failed to set file as executable. (%s)",
