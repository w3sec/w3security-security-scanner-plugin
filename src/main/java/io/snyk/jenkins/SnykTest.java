...
public class W3SecurityTest {
  ...
  public static Result testProject(
    ...
    FilePath stdoutPath = workspace.child(
      getURLSafeDateTime() + "_w3security_report.json"
    );
    ...
    Map<String, String> commandEnvVars = CommandLine.asEnvVars(
      w3securityToken,
      envVars
    );
    ...
    W3SecurityTestResult result = ObjectMapperHelper.unmarshallTestResult(
      stdout
    );
    if (result == null) {
    ...
    if (exitCode >= 2) {
      throw new RuntimeException(
        "An error occurred. (Exit Code: " + exitCode + ")"
      );
    }
    if (!result.ok) {
