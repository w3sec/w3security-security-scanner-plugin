...
public class W3SecurityStepBuilder
  extends Builder
  implements SimpleBuildStep, W3SecurityConfig {

  private static final Logger LOG = LoggerFactory.getLogger(
    W3SecurityStepBuilder.class.getName()
  );
  ...
  public void perform(
    ...
    W3SecurityStepFlow.perform(
      this,
      () ->
        W3SecurityContext.forFreestyleProject(build, workspace, launcher, log)
    );
  }

  @Extension
  public static class W3SecurityStepBuilderDescriptor
    ...
    public boolean hasInstallationsAvailable() {
      if (LOG.isTraceEnabled()) {
        LOG.trace(
          "Available W3Security installations: {}",
          Arrays
            .stream(installations)
            .map(W3SecurityInstallation::getName)
            .collect(joining(",", "[", "]"))
        );
      }
    ...
    public ListBoxModel doFillW3SecurityTokenIdItems(
      @AncestorInPath Item item,
      @QueryParameter String w3securityTokenId
    ) {
    ...
    public FormValidation doCheckSeverity(
      @QueryParameter String value,
      @QueryParameter String additionalArguments
    ) {
      if (
        fixEmptyAndTrim(value) == null ||
        fixEmptyAndTrim(additionalArguments) == null
      ) {
    ...
    public FormValidation doCheckW3SecurityTokenId(
      @AncestorInPath Item item,
      @QueryParameter String value
    ) {
      if (item == null) {
      ...
      if (
        ...
        CredentialsMatchers.firstOrNull(
          ...
          anyOf(
            withId(value),
            CredentialsMatchers.instanceOf(W3SecurityApiToken.class)
          )
        )
      ) {
        return FormValidation.error(
          "Cannot find currently selected W3Security API token."
        );
      }
      return FormValidation.ok();
    }

    public FormValidation doCheckTargetFile(
      @QueryParameter String value,
      @QueryParameter String additionalArguments
    ) {
      if (
      ...
      if (additionalArguments.contains("--file")) {
        return FormValidation.warning(
          "Option '--file' is overridden in additional arguments text area below."
        );
      }
      return FormValidation.ok();
    }
    ...
    public FormValidation doCheckOrganisation(
      ...
      if (additionalArguments.contains("--org")) {
        return FormValidation.warning(
          "Option '--org' is overridden in additional arguments text area below."
        );
      }
      return FormValidation.ok();
    }
    ...
    public FormValidation doCheckProjectName(
      ...
      if (
        fixEmptyAndTrim(value) == null ||
        fixEmptyAndTrim(monitorProjectOnBuild) == null
      ) {
        return FormValidation.ok();
      }
