...
public class W3SecurityStepFlow {
  ...
  public static void perform(
    ...
    Supplier<W3SecurityContext> contextSupplier
    ...
    try {
      ...
      String w3securityToken = W3SecurityApiToken.getToken(
        context,
        config.getW3SecurityTokenId()
      );

      foundIssues =
        W3SecurityStepFlow.testProject(
          context,
          config,
          installation,
          w3securityToken
      ...
      if (context != null) {
        ...
        ex.printStackTrace(
          listener.fatalError("W3Security Security scan failed.")
        );
      }
      cause = ex;
    }
  ...
  private static boolean testProject(
    W3SecurityContext context,
    W3SecurityConfig config,
    W3SecurityInstallation installation,
    String w3securityToken
  ) throws IOException, InterruptedException {
    W3SecurityTest.Result testResult = W3SecurityTest.testProject(
      context,
      config,
      installation,
      w3securityToken
    );
    FilePath report = W3SecurityToHTML.generateReport(
      context,
      installation,
      testResult.testJsonPath
    );
  ...
  private static void archiveReport(W3SecurityContext context, FilePath report)
    throws IOException, InterruptedException {
